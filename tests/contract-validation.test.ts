...
import{validateSynchronizationCommand}from"../src/synchronization/validation.ts";
import{validateAuditCommand}from"../src/audit/validation.ts";
import{validatePolicyCommand}from"../src/policy/validation.ts";
...
describe("strict command envelope validation", () => {
  it("accepts valid commands from every executable context", () => {
    ...
    validateSynchronizationCommand({...conversationCommand("CreateConversation",8,{conversation_id:testId(957),title:"x",creator_id:testId(800)},"communication:create",0),command_type:"StartSynchronization",target:{aggregate_type:"Synchronization",object_id:testId(957)},payload:{synchronization_id:testId(957),subject_ref:{aggregate_type:"Task",object_id:testId(400)},source_replica_id:"local",target_replica_id:"remote",base_vector_clock:{local:1}},authority_proof:{...conversationCommand("CreateConversation",8,{conversation_id:testId(957),title:"x",creator_id:testId(800)},"communication:create",0).authority_proof,scope:["synchronization:start"]}});
    validateAuditCommand({...conversationCommand("CreateConversation",9,{conversation_id:testId(958),title:"x",creator_id:testId(800)},"communication:create",0),command_type:"AppendAuditEntry",target:{aggregate_type:"AuditPartition",object_id:testId(958)},payload:{audit_partition_id:testId(958),entry_id:testId(959),subject_ref:{aggregate_type:"Task",object_id:testId(400)},action:"Created",actor_id:testId(800),occurred_at:"2026-07-29T20:00:00.000000Z",integrity_digest:"a".repeat(64)},authority_proof:{...conversationCommand("CreateConversation",9,{conversation_id:testId(958),title:"x",creator_id:testId(800)},"communication:create",0).authority_proof,scope:["audit:entry:append"]}});
    validatePolicyCommand({...conversationCommand("CreateConversation",10,{conversation_id:testId(960),title:"x",creator_id:testId(800)},"communication:create",0),command_type:"CreatePolicy",target:{aggregate_type:"Policy",object_id:testId(960)},payload:{policy_id:testId(960),name:"Access",description:"Access controls",policy_type:"ACCESS",owner_id:testId(800)},authority_proof:{...conversationCommand("CreateConversation",10,{conversation_id:testId(960),title:"x",creator_id:testId(800)},"communication:create",0).authority_proof,scope:["policy:create"]}});
  });
