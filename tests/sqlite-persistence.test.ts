...
import{PolicyService}from"../src/policy/service.ts";import{SqlitePolicyRepository}from"../src/policy/sqlite-repository.ts";
...
import { contextLinkCommand, conversationCommand, createMissionCommand, createReportCommand, createTaskCommand, createTimelineCommand, identityCommand, meetingCommand, missionCommand, organizationCommand, testId, timelineCommand } from "./fixtures.ts";
...
describe("SQLite persistence", () => {
  ...
  it("restores Policy versions and idempotency after restart",async()=>{const directory=await mkdtemp(join(tmpdir(),"onyx-sqlite-policy-")),path=join(directory,"onyx.db"),id=testId(993),base=conversationCommand("CreateConversation",840,{conversation_id:id,title:"x",creator_id:testId(800)},"communication:create",0),create={...base,command_type:"CreatePolicy",target:{aggregate_type:"Policy",object_id:id},payload:{policy_id:id,name:"Retention",description:"Retention controls",policy_type:"RETENTION",owner_id:testId(800)},authority_proof:{...base.authority_proof,scope:["policy:create"]}};try{const db1=new SqliteDatabase(path),first=new PolicyService({repository:new SqlitePolicyRepository(db1),now,requireUser:async()=>undefined,requireObject:async()=>undefined}),created=await first.execute(create),version={...create,command_id:testId(841),operation_id:testId(842),command_type:"CreatePolicyVersion",expected_version:1,payload:{policy_id:id,version_id:testId(994),change_note:"initial",rules:[{rule_id:testId(995),effect:"ALLOW",resource_type:"FileAsset",action:"READ",condition:"always"}]},authority_proof:{...create.authority_proof,scope:["policy:version:create"]}};await first.execute(version);db1.close();const db2=new SqliteDatabase(path),restored=new PolicyService({repository:new SqlitePolicyRepository(db2),now,requireUser:async()=>undefined,requireObject:async()=>undefined});assert.equal(Object.keys((await restored.getPolicy(testId(13),id)).versions).length,1);assert.deepEqual(await restored.execute(create),created);db2.close()}finally{await rm(directory,{recursive:true,force:true})}});
  it("persists due Timeline signals exactly once across restart",async()=>{const directory=await mkdtemp(join(tmpdir(),"onyx-sqlite-timeline-signals-")),path=join(directory,"onyx.db"),id=testId(500);let current=new Date("2026-07-29T20:00:01.000Z");try{const db1=new SqliteDatabase(path),first=new TimelineService({repository:new SqliteTimelineRepository(db1),requireSubject:async()=>undefined,now:()=>current});await first.execute(createTimelineCommand());await first.execute(timelineCommand("SetDeadline",90,{timeline_id:id,deadline_id:testId(996),deadline_at:"2026-07-29T20:00:02.000000Z",label:"Persistent due"},"timeline:deadline:set",1));current=new Date("2026-07-29T20:00:03.000Z");assert.deepEqual((await first.processDueSignals(testId(13),id)).map(e=>e.event_type),["DeadlineReached"]);db1.close();const db2=new SqliteDatabase(path),restored=new TimelineService({repository:new SqliteTimelineRepository(db2),requireSubject:async()=>undefined,now:()=>new Date("2026-07-29T20:00:04.000Z")});assert.deepEqual(await restored.processDueSignals(testId(13),id),[]);assert.deepEqual((await restored.getHistory(testId(13),id)).map(e=>e.event_type).slice(-1),["DeadlineReached"]);db2.close()}finally{await rm(directory,{recursive:true,force:true})}});
