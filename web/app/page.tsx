  ...
  return <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="mission-drawer" role="dialog" aria-modal="true" aria-label="Mission details">
      ...
      <div className="drawer-scroll">
        ...
        <section className="lifecycle-card"><div><p className="eyebrow">LIFECYCLE CONTROL</p><h3>Available operation</h3></div>{primaryAction ? <button className="primary-button" disabled={Boolean(actionLoading)} onClick={() => onAction(primaryAction[0] as "plan" | "submit" | "activate" | "pause" | "resume" | "archive")}>{actionLoading === primaryAction[0] ? "Working…" : primaryAction[1]}</button> : <span className="status neutral">No transition</span>}</section>
        {!["CANCELLED", "ARCHIVED"].includes(mission.status) && <button className="danger-action" disabled={Boolean(actionLoading)} onClick={() => onAction("cancel")}>Cancel mission</button>}
...
function RecordDetail({ selection, history, missions, tasks, timelines, reports, loading, onOpenMission, onOpenRecord, onClose }: { selection: RecordSelection; history: DomainEvent[]; missions: Mission[]; tasks: Task[]; timelines: Timeline[]; reports: Report[]; loading: boolean; onOpenMission: (mission: Mission) => void; onOpenRecord: (selection: RecordSelection) => void; onClose: () => void }) {
  let id = "";
  let title = "";
  let description = "";
  let badge = "";
