...
type Forecast={forecast_id:string;organization_id:string;title:string;subject_ref:{aggregate_type:string;object_id:string};horizon_start:string;horizon_end:string;method:string;baseline_value:number;projected_value:number;status:string;version:number;lifecycle_epoch:number;authority_epoch:number;scenarios:Record<string,{scenario_id:string;name:string;probability:number;adjustments:Record<string,number>;projected_value:number}>;calculated_at?:string;published_at?:string};
type AutomationRule={automation_rule_id:string;organization_id:string;name:string;owner_id:string;trigger_type:string;trigger_expression:string;action_type:string;action_config:Record<string,string>;status:string;version:number;lifecycle_epoch:number;authority_epoch:number;evaluations:Record<string,{evaluation_id:string;input:Record<string,string>}>;executions:Record<string,{execution_id:string;evaluation_id:string;status:string;result:Record<string,string>;retry_attempt:number}>};
type Notification={notification_id:string;organization_id:string;title:string;body:string;severity:string;created_by_id:string;source_ref?:{aggregate_type:string;object_id:string};status:string;version:number;lifecycle_epoch:number;authority_epoch:number;recipients:Record<string,{recipient_id:string;channels:string[];acknowledged_at?:string}>;deliveries:Record<string,{delivery_batch_id:string;sent_at:string;status:string;retry_attempt:number}>;escalations:Array<{escalated_to_id:string;reason:string}>};
type Synchronization={synchronization_id:string;organization_id:string;subject_ref:{aggregate_type:string;object_id:string};source_replica_id:string;target_replica_id:string;vector_clock:Record<string,number>;status:string;version:number;lifecycle_epoch:number;authority_epoch:number;batches:Record<string,{batchId:string;operationIds:string[];acceptedOperationIds:string[];status:string}>;conflicts:Record<string,{conflictId:string;status:string;localValue:string;remoteValue:string}>;acknowledgements:Record<string,Record<string,number>>};
type AuditPartition={audit_partition_id:string;organization_id:string;status:string;version:number;lifecycle_epoch:number;authority_epoch:number;entries:Array<{entry_id:string;sequence:number;action:string}>;exports:Record<string,unknown>;verifications:Record<string,{valid:boolean}>;root_digest:string;sealed_at?:string};
type Policy={policy_id:string;organization_id:string;name:string;description:string;policy_type:string;owner_id:string;status:string;version:number;lifecycle_epoch:number;authority_epoch:number;versions:Record<string,{versionId:string;status:string;rules:Array<{rule_id:string;effect:string;resource_type:string;action:string}>}>;active_version_id?:string;evaluations:Record<string,{evaluationId:string;decision:string;signal?:string;subjectRef:{aggregate_type:string;object_id:string}}> ;violations:Record<string,unknown>;holds:Record<string,{hold_id:string;status:string}>;rate_limits:Record<string,{rate_limit_id:string;max_requests:number;threshold_percent:number}>};
const POLICY_EVENT_LABELS=["QuotaThresholdReached","QuotaExceeded","RateLimitTriggered"] as const;
...
export default function Home() {
  ...
  const[auditPartitions,setAuditPartitions]=useState<AuditPartition[]>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const [healthData, missionData, taskData, timelineData, reportData, organizationData, userData, contextLinkData,meetingData,conversationData,fileData,approvalData,capacityData,forecastData,automationData,notificationData,synchronizationData,auditData,policyData] = await Promise.all([
        api<Ready>("/readyz"),
        ...
        api<ResourcePage<Forecast>>(`/v1/forecasts?${query}`),
        api<ResourcePage<AutomationRule>>(`/v1/automation-rules?${query}`),
        api<ResourcePage<Notification>>(`/v1/notifications?${query}`),
        api<ResourcePage<Synchronization>>(`/v1/synchronizations?${query}`),
        api<ResourcePage<AuditPartition>>(`/v1/audit-partitions?${query}`),
        api<ResourcePage<Policy>>(`/v1/policies?${query}`),
      ]);
      setReady(healthData);
      ...
      setForecasts(forecastData.items);
      setAutomationRules(automationData.items);
      setNotifications(notificationData.items);
      setSynchronizations(synchronizationData.items);
      setAuditPartitions(auditData.items);
  ...
  async function runTaskAction(type: "owner"|"priority"|"dependency"|"start" | "pause" | "block" | "submit" | "approve" | "close" | "reopen" | "cancel") {
    ...
    try {
      const config = {
        owner:["AssignOwner","work:owner:assign",{task_id:task.task_id,owner_id:users.find(u=>u.status==="ACTIVE"&&u.user_id!==task.owner_id)?.user_id??principalId,assignment_note:"Assigned from ONYX Command Center"}],
        priority:["ChangePriority","work:priority:change",{task_id:task.task_id,priority:task.priority==="CRITICAL"?"HIGH":"CRITICAL",reason:"Operational reprioritization"}],
        dependency:["AddDependency","work:dependency:add",{task_id:task.task_id,dependency_task_id:tasks.find(t=>t.mission_id===task.mission_id&&t.task_id!==task.task_id&&!task.dependency_task_ids.includes(t.task_id))?.task_id??""}],
      ...
      if(type==="dependency"&&!(payload as {dependency_task_id?:string}).dependency_task_id)throw new Error("Create another task in this mission first");
  ...
  async function runForecastAction(type:"generate"|"scenario"|"recalculate"|"publish"|"archive",selected?:Forecast){setActionLoading(`forecast:${type}`);try{const item=selected??forecasts.find(f=>f.status!=="ARCHIVED")??forecasts[0];let commandType:string,scope:string,id:string,payload:Record<string,unknown>;if(type==="generate"){const subject=capacityProfiles[0]?{aggregate_type:"CapacityProfile",object_id:capacityProfiles[0].capacity_profile_id}:missions[0]?{aggregate_type:"Mission",object_id:missions[0].mission_id}:undefined;if(!subject)throw new Error("Create a capacity profile or mission first");id=uuidV7();commandType="GenerateForecast";scope="forecast:generate";payload={forecast_id:id,title:`Operational outlook ${forecasts.length+1}`,subject_ref:subject,horizon_start:utcNow(),horizon_end:new Date(Date.now()+30*86_400_000).toISOString().replace(/Z$/,"000Z"),method:"EXPERT",baseline_value:100};}else{if(!item||item.status==="ARCHIVED")throw new Error("Generate an active forecast first");id=item.forecast_id;if(type==="scenario"){commandType="CreateScenario";scope="forecast:scenario:create";payload={forecast_id:id,scenario_id:uuidV7(),name:`Scenario ${Object.keys(item.scenarios).length+1}`,probability:.5,adjustments:{capacity:20,risk:-5}};}else if(type==="recalculate"){commandType="RecalculateForecast";scope="forecast:recalculate";payload={forecast_id:id,as_of:utcNow()};}else if(type==="publish"){commandType="PublishForecast";scope="forecast:publish";payload={forecast_id:id,published_at:utcNow()};}else{commandType="ArchiveForecast";scope="forecast:archive";payload={forecast_id:id,reason:"Forecast horizon complete"};}}const command=envelope(commandType,"Forecast",id,organizationId,principalId,scope,payload);if(type!=="generate"&&item){command.expected_version=item.version;command.expected_lifecycle_epoch=item.lifecycle_epoch;command.expected_authority_epoch=item.authority_epoch;}await api(`/v1/forecasting/commands/${commandType}`,{method:"POST",body:JSON.stringify(command)});await refresh();setToast(`${commandType} completed`);}catch(caught){setToast(caught instanceof Error?caught.message:"Forecast action failed");}finally{setActionLoading("");}}

  async function runAutomationAction(type:"create"|"enable"|"disable"|"evaluate"|"execute"|"retry"|"compensate"|"archive",selected?:AutomationRule){setActionLoading(`automation:${type}`);try{const rule=selected??automationRules.find(r=>r.status!=="ARCHIVED")??automationRules[0];let commandType:string,scope:string,id:string,payload:Record<string,unknown>;if(type==="create"){const owner=users.find(user=>user.status==="ACTIVE");if(!owner)throw new Error("Create an active user first");id=uuidV7();commandType="CreateAutomationRule";scope="automation:create";payload={automation_rule_id:id,name:`Blocked task notifier ${automationRules.length+1}`,owner_id:owner.user_id,trigger_type:"EVENT",trigger_expression:"TaskBlocked",action_type:"NOTIFICATION",action_config:{channel:"operations"}};}else{if(!rule||rule.status==="ARCHIVED")throw new Error("Create an active automation rule first");id=rule.automation_rule_id;const evaluation=Object.values(rule.evaluations).at(-1),execution=Object.values(rule.executions).at(-1);if(type==="enable"||type==="disable"){commandType=type==="enable"?"EnableRule":"DisableRule";scope=type==="enable"?"automation:enable":"automation:disable";payload={automation_rule_id:id,reason:`${type} from Command Center`};}else if(type==="evaluate"){commandType="EvaluateRule";scope="automation:evaluate";payload={automation_rule_id:id,evaluation_id:uuidV7(),input:{event_type:"TaskBlocked"}};}else if(type==="execute"){if(!evaluation)throw new Error("Evaluate the rule first");commandType="ExecuteAction";scope="automation:execute";payload={automation_rule_id:id,execution_id:uuidV7(),evaluation_id:evaluation.evaluation_id,result:{delivery:"accepted"}};}else if(type==="retry"){if(!execution)throw new Error("Execute an action first");commandType="RetryExecution";scope="automation:retry";payload={automation_rule_id:id,execution_id:execution.execution_id,retry_attempt:execution.retry_attempt+1,failure_reason:"Downstream timeout"};}else if(type==="compensate"){if(!execution)throw new Error("Execute an action first");commandType="CompensateExecution";scope="automation:compensate";payload={automation_rule_id:id,execution_id:execution.execution_id,reason:"Action withdrawn"};}else{commandType="ArchiveRule";scope="automation:archive";payload={automation_rule_id:id,reason:"Rule superseded"};}}const command=envelope(commandType,"AutomationRule",id,organizationId,principalId,scope,payload);if(type!=="create"&&rule){command.expected_version=rule.version;command.expected_lifecycle_epoch=rule.lifecycle_epoch;command.expected_authority_epoch=rule.authority_epoch;}await api(`/v1/automation/commands/${commandType}`,{method:"POST",body:JSON.stringify(command)});await refresh();setToast(`${commandType} completed`);}catch(caught){setToast(caught instanceof Error?caught.message:"Automation action failed");}finally{setActionLoading("");}}

  async function runNotificationAction(type:"create"|"resolve"|"send"|"retry"|"escalate"|"acknowledge"|"archive",selected?:Notification){setActionLoading(`notification:${type}`);try{const item=selected??notifications.find(n=>n.status!=="ARCHIVED")??notifications[0];let commandType:string,scope:string,id:string,payload:Record<string,unknown>;if(type==="create"){const creator=users.find(user=>user.status==="ACTIVE"),source=automationRules[0]?{aggregate_type:"AutomationRule",object_id:automationRules[0].automation_rule_id}:tasks[0]?{aggregate_type:"Task",object_id:tasks[0].task_id}:undefined;if(!creator)throw new Error("Create an active user first");id=uuidV7();commandType="CreateNotification";scope="notification:create";payload={notification_id:id,title:`Operations notice ${notifications.length+1}`,body:"A monitored operational condition requires attention.",severity:"WARNING",created_by_id:creator.user_id,...(source?{source_ref:source}:{})};}else{if(!item||item.status==="ARCHIVED")throw new Error("Create an active notification first");id=item.notification_id;const delivery=Object.values(item.deliveries).at(-1),recipient=Object.values(item.recipients).find(r=>!r.acknowledged_at);if(type==="resolve"){const ids=users.filter(u=>u.status==="ACTIVE").slice(0,3).map(u=>u.user_id);if(!ids.length)throw new Error("Create an active user first");commandType="ResolveRecipients";scope="notification:recipients:resolve";payload={notification_id:id,recipient_ids:ids,channels:["IN_APP","EMAIL"]};}else if(type==="send"){commandType="SendNotification";scope="notification:send";payload={notification_id:id,delivery_batch_id:uuidV7(),sent_at:utcNow()};}else if(type==="retry"){if(!delivery)throw new Error("Send the notification first");commandType="RetryDelivery";scope="notification:delivery:retry";payload={notification_id:id,delivery_batch_id:delivery.delivery_batch_id,retry_attempt:delivery.retry_attempt+1,failure_reason:"Channel timeout"};}else if(type==="escalate"){const user=users.find(u=>u.status==="ACTIVE"&&!item.recipients[u.user_id]);if(!user)throw new Error("Create another active user to escalate");commandType="EscalateNotification";scope="notification:escalate";payload={notification_id:id,escalated_to_id:user.user_id,reason:"Acknowledgement SLA exceeded"};}else if(type==="acknowledge"){if(!recipient)throw new Error("Resolve an unacknowledged recipient first");commandType="AcknowledgeNotification";scope="notification:acknowledge";payload={notification_id:id,recipient_id:recipient.recipient_id,acknowledged_at:utcNow()};}else{commandType="ArchiveNotification";scope="notification:archive";payload={notification_id:id,reason:"Notification resolved"};}}const command=envelope(commandType,"Notification",id,organizationId,principalId,scope,payload);if(type!=="create"&&item){command.expected_version=item.version;command.expected_lifecycle_epoch=item.lifecycle_epoch;command.expected_authority_epoch=item.authority_epoch;}await api(`/v1/notification/commands/${commandType}`,{method:"POST",body:JSON.stringify(command)});await refresh();setToast(`${commandType} completed`);}catch(caught){setToast(caught instanceof Error?caught.message:"Notification action failed");}finally{setActionLoading("");}}

  async function runSynchronizationAction(type:"start"|"offer"|"accept"|"merge"|"resolve"|"escalate"|"acknowledge"|"close",selected?:Synchronization){setActionLoading(`synchronization:${type}`);try{const item=selected??synchronizations.find(s=>s.status!=="CLOSED")??synchronizations[0];let commandType:string,scope:string,id:string,payload:Record<string,unknown>;if(type==="start"){const subject=tasks[0]?{aggregate_type:"Task",object_id:tasks[0].task_id}:missions[0]?{aggregate_type:"Mission",object_id:missions[0].mission_id}:undefined;if(!subject)throw new Error("Create a task or mission first");id=uuidV7();commandType="StartSynchronization";scope="synchronization:start";payload={synchronization_id:id,subject_ref:subject,source_replica_id:"command-center",target_replica_id:"edge-replica",base_vector_clock:{"command-center":1}};}else{if(!item||item.status==="CLOSED")throw new Error("Start an active synchronization first");id=item.synchronization_id;const offered=Object.values(item.batches).find(b=>b.status==="OFFERED"),accepted=Object.values(item.batches).find(b=>b.status==="ACCEPTED"),conflict=Object.values(item.conflicts).find(c=>c.status!=="RESOLVED");if(type==="offer"){commandType="OfferOperationBatch";scope="synchronization:batch:offer";payload={synchronization_id:id,batch_id:uuidV7(),operation_ids:[uuidV7()],offered_vector_clock:{"command-center":item.version+1}};}else if(type==="accept"){if(!offered)throw new Error("Offer a batch first");commandType="AcceptOperationBatch";scope="synchronization:batch:accept";payload={synchronization_id:id,batch_id:offered.batchId,accepted_operation_ids:offered.operationIds};}else if(type==="merge"){if(!accepted)throw new Error("Accept a batch first");commandType="MergeOperationBatch";scope="synchronization:batch:merge";payload={synchronization_id:id,batch_id:accepted.batchId,merged_vector_clock:{"command-center":item.version+1,"edge-replica":1},conflicts:[{conflict_id:uuidV7(),field_path:"status",local_value:"ACTIVE",remote_value:"PAUSED"}]};}else if(type==="resolve"){if(!conflict)throw new Error("Merge a conflicting batch first");commandType="ResolveConflict";scope="synchronization:conflict:resolve";payload={synchronization_id:id,conflict_id:conflict.conflictId,resolution:"KEEP_LOCAL",resolved_value:conflict.localValue};}else if(type==="escalate"){if(!conflict)throw new Error("Merge a conflicting batch first");const user=users.find(u=>u.status==="ACTIVE");if(!user)throw new Error("Create an active user first");commandType="EscalateConflict";scope="synchronization:conflict:escalate";payload={synchronization_id:id,conflict_id:conflict.conflictId,escalated_to_id:user.user_id,reason:"Manual convergence review"};}else if(type==="acknowledge"){const replica=item.acknowledgements[item.source_replica_id]?item.target_replica_id:item.source_replica_id;commandType="AcknowledgeSynchronization";scope="synchronization:acknowledge";payload={synchronization_id:id,replica_id:replica,acknowledged_vector_clock:item.vector_clock};}else{commandType="CloseSynchronization";scope="synchronization:close";payload={synchronization_id:id,reason:"Replicas converged"};}}const command=envelope(commandType,"Synchronization",id,organizationId,principalId,scope,payload);if(type!=="start"&&item){command.expected_version=item.version;command.expected_lifecycle_epoch=item.lifecycle_epoch;command.expected_authority_epoch=item.authority_epoch;}await api(`/v1/synchronization/commands/${commandType}`,{method:"POST",body:JSON.stringify(command)});await refresh();setToast(`${commandType} completed`);}catch(caught){setToast(caught instanceof Error?caught.message:"Synchronization action failed");}finally{setActionLoading("");}}

  async function runAuditAction(type:"append"|"seal"|"export"|"verify"|"archive",selected?:AuditPartition){setActionLoading(`audit:${type}`);try{const item=selected??auditPartitions.find(p=>p.status!=="ARCHIVED")??auditPartitions[0];let commandType:string,scope:string,id:string,payload:Record<string,unknown>;if(type==="append"&&(!item||item.status!=="OPEN")){const subject=tasks[0]?{aggregate_type:"Task",object_id:tasks[0].task_id}:missions[0]?{aggregate_type:"Mission",object_id:missions[0].mission_id}:undefined;if(!subject)throw new Error("Create a task or mission first");id=uuidV7();commandType="AppendAuditEntry";scope="audit:entry:append";payload={audit_partition_id:id,entry_id:uuidV7(),subject_ref:subject,action:"CommandCenterObserved",actor_id:principalId,occurred_at:utcNow(),integrity_digest:"0".repeat(64)};}else{if(!item)throw new Error("Append an audit entry first");id=item.audit_partition_id;if(type==="append"){commandType="AppendAuditEntry";scope="audit:entry:append";payload={audit_partition_id:id,entry_id:uuidV7(),subject_ref:{aggregate_type:"AuditPartition",object_id:id},action:"AuditEntryObserved",actor_id:principalId,occurred_at:utcNow(),integrity_digest:"0".repeat(64)};}else if(type==="seal"){commandType="SealAuditPartition";scope="audit:partition:seal";payload={audit_partition_id:id,sealed_at:utcNow(),expected_entry_count:item.entries.length};}else if(type==="export"){commandType="CreateAuditExport";scope="audit:export:create";payload={audit_partition_id:id,export_id:uuidV7(),format:"JSONL",from_sequence:1,to_sequence:item.entries.length};}else if(type==="verify"){commandType="VerifyIntegrity";scope="audit:integrity:verify";payload={audit_partition_id:id,verification_id:uuidV7(),expected_root_digest:item.root_digest};}else{commandType="ArchiveAuditPartition";scope="audit:partition:archive";payload={audit_partition_id:id,reason:"Retention cycle completed"};}}const command=envelope(commandType,"AuditPartition",id,organizationId,principalId,scope,payload);if(item&&id===item.audit_partition_id){command.expected_version=item.version;command.expected_lifecycle_epoch=item.lifecycle_epoch;command.expected_authority_epoch=item.authority_epoch;}await api(`/v1/audit/commands/${commandType}`,{method:"POST",body:JSON.stringify(command)});await refresh();setToast(`${commandType} completed`);}catch(caught){setToast(caught instanceof Error?caught.message:"Audit action failed");}finally{setActionLoading("");}}

  async function runPolicyAction(type:"create"|"version"|"publish"|"rate"|"evaluate"|"violation"|"hold"|"release"|"retire",selected?:Policy){setActionLoading(`policy:${type}`);try{const item=selected??policies.find(p=>p.status!=="RETIRED")??policies[0];let commandType:string,scope:string,id:string,payload:Record<string,unknown>;if(type==="create"){const owner=users.find(u=>u.status==="ACTIVE");if(!owner)throw new Error("Create an active user first");id=uuidV7();commandType="CreatePolicy";scope="policy:create";payload={policy_id:id,name:`API governance ${policies.length+1}`,description:"Decision, quota, legal hold, and rate limit controls.",policy_type:"RATE_LIMIT",owner_id:owner.user_id};}else{if(!item||item.status==="RETIRED")throw new Error("Create an active policy first");id=item.policy_id;const draft=Object.values(item.versions).find(v=>v.status==="DRAFT"),rate=Object.values(item.rate_limits)[0],evaluation=Object.values(item.evaluations)[0],hold=Object.values(item.holds).find(h=>h.status==="ACTIVE"),subject=tasks[0]?{aggregate_type:"Task",object_id:tasks[0].task_id}:missions[0]?{aggregate_type:"Mission",object_id:missions[0].mission_id}:undefined;if(type==="version"){commandType="CreatePolicyVersion";scope="policy:version:create";payload={policy_id:id,version_id:uuidV7(),change_note:"Command Center policy revision",rules:[{rule_id:uuidV7(),effect:"DENY",resource_type:"Task",action:"DELETE",condition:"always"}]};}else if(type==="publish"){if(!draft)throw new Error("Create a draft policy version first");commandType="PublishPolicyVersion";scope="policy:version:publish";payload={policy_id:id,version_id:draft.versionId,published_at:utcNow()};}else if(type==="rate"){commandType="DefineRateLimitPolicy";scope="policy:rate-limit:define";payload={policy_id:id,rate_limit_id:uuidV7(),scope:"command-center",window_seconds:60,max_requests:100,quota_limit:1000,threshold_percent:80};}else if(type==="evaluate"){if(!subject)throw new Error("Create a task or mission first");const count=Object.keys(item.evaluations).length,signal=count%3;commandType="EvaluatePolicy";scope="policy:evaluate";payload={policy_id:id,evaluation_id:uuidV7(),subject_ref:{aggregate_type:"User",object_id:principalId},resource_ref:subject,action:"DELETE",attributes:{channel:"ui"},...(rate?{rate_limit_id:rate.rate_limit_id,observed_requests:signal===2?101:50,observed_quota_percent:signal===0?85:signal===1?100:50}:{})};}else if(type==="violation"){if(!evaluation||!subject)throw new Error("Evaluate the policy first");commandType="RegisterViolation";scope="policy:violation:register";payload={policy_id:id,violation_id:uuidV7(),evaluation_id:evaluation.evaluationId,subject_ref:subject,severity:"HIGH",description:"Denied policy action observed"};}else if(type==="hold"){if(!subject)throw new Error("Create a task or mission first");commandType="ApplyLegalHold";scope="policy:legal-hold:apply";payload={policy_id:id,hold_id:uuidV7(),subject_ref:subject,reason:"Investigation retention",applied_by_id:principalId};}else if(type==="release"){if(!hold)throw new Error("Apply an active legal hold first");commandType="ReleaseLegalHold";scope="policy:legal-hold:release";payload={policy_id:id,hold_id:hold.hold_id,released_by_id:principalId,reason:"Investigation completed"};}else{commandType="RetirePolicy";scope="policy:retire";payload={policy_id:id,reason:"Policy superseded"};}}const command=envelope(commandType,"Policy",id,organizationId,principalId,scope,payload);if(type!=="create"&&item){command.expected_version=item.version;command.expected_lifecycle_epoch=item.lifecycle_epoch;command.expected_authority_epoch=item.authority_epoch;}await api(`/v1/policy/commands/${commandType}`,{method:"POST",body:JSON.stringify(command)});await refresh();setToast(`${commandType} completed`);}catch(caught){setToast(caught instanceof Error?caught.message:"Policy action failed");}finally{setActionLoading("");}}
        ...
        <div className="content">
          ...
          {view === "overview" && (
            <>
              ...
              <ForecastPanel forecasts={forecasts} loading={actionLoading} onAction={(type,forecast)=>void runForecastAction(type,forecast)}/>
              <AutomationPanel rules={automationRules} loading={actionLoading} onAction={(type,rule)=>void runAutomationAction(type,rule)}/>
              <NotificationPanel notifications={notifications} loading={actionLoading} onAction={(type,item)=>void runNotificationAction(type,item)}/>
              <SynchronizationPanel synchronizations={synchronizations} loading={actionLoading} onAction={(type,item)=>void runSynchronizationAction(type,item)}/>
              <AuditPanel partitions={auditPartitions} loading={actionLoading} onAction={(type,item)=>void runAuditAction(type,item)}/>
              <PolicyPanel policies={policies} loading={actionLoading} onAction={(type,item)=>void runPolicyAction(type,item)}/>
...
function ForecastPanel({forecasts,loading,onAction}:{forecasts:Forecast[];loading:string;onAction:(type:"generate"|"scenario"|"recalculate"|"publish"|"archive",forecast?:Forecast)=>void}){const item=forecasts.find(f=>f.status!=="ARCHIVED")??forecasts[0];return <section className="panel"><div className="identity-heading"><div><p className="eyebrow">FORECAST LAB</p><h2>Scenario outlook</h2><p>Probability-weighted projections with explicit recalculation and publication gates.</p></div><button className="primary-button" disabled={Boolean(loading)} onClick={()=>onAction("generate")}>＋ Generate forecast</button></div>{item?<div className="organization-overview"><article><small>Forecast</small><strong>{item.title}</strong><span>{item.method} · {item.status}</span></article><article><small>Baseline</small><strong>{item.baseline_value}</strong><span>{item.subject_ref.aggregate_type}</span></article><article><small>Projected</small><strong>{item.projected_value.toFixed(1)}</strong><span>{item.calculated_at?"calculated":"draft model"}</span></article><article><small>Scenarios</small><strong>{Object.keys(item.scenarios).length}</strong><span>epoch {item.lifecycle_epoch}</span></article></div>:<p className="organization-placeholder">No forecasts exist yet.</p>}{item&&item.status!=="ARCHIVED"&&<div className="organization-actions"><button onClick={()=>onAction("scenario",item)}>Add scenario</button><button onClick={()=>onAction("recalculate",item)}>Recalculate</button><button onClick={()=>onAction("publish",item)}>Publish</button><button className="danger-control" onClick={()=>onAction("archive",item)}>Archive</button></div>}</section>}
function AutomationPanel({rules,loading,onAction}:{rules:AutomationRule[];loading:string;onAction:(type:"create"|"enable"|"disable"|"evaluate"|"execute"|"retry"|"compensate"|"archive",rule?:AutomationRule)=>void}){const rule=rules.find(r=>r.status!=="ARCHIVED")??rules[0],execution=rule&&Object.values(rule.executions).at(-1);return <section className="panel"><div className="identity-heading"><div><p className="eyebrow">AUTOMATION ENGINE</p><h2>Rules and executions</h2><p>Stateful triggers, action receipts, retry failures, and explicit compensation.</p></div><button className="primary-button" disabled={Boolean(loading)} onClick={()=>onAction("create")}>＋ Create rule</button></div>{rule?<div className="organization-overview"><article><small>Rule</small><strong>{rule.name}</strong><span>{rule.trigger_type} · {rule.status}</span></article><article><small>Action</small><strong>{rule.action_type}</strong><span>{rule.trigger_expression}</span></article><article><small>Evaluations</small><strong>{Object.keys(rule.evaluations).length}</strong><span>{Object.keys(rule.executions).length} executions</span></article><article><small>Latest receipt</small><strong>{execution?.status??"NONE"}</strong><span>{execution?`retry ${execution.retry_attempt}`:`epoch ${rule.lifecycle_epoch}`}</span></article></div>:<p className="organization-placeholder">No automation rules exist yet.</p>}{rule&&rule.status!=="ARCHIVED"&&<div className="organization-actions"><button onClick={()=>onAction("enable",rule)}>Enable</button><button onClick={()=>onAction("disable",rule)}>Disable</button><button onClick={()=>onAction("evaluate",rule)}>Evaluate</button><button onClick={()=>onAction("execute",rule)}>Execute</button><button onClick={()=>onAction("retry",rule)}>Retry</button><button onClick={()=>onAction("compensate",rule)}>Compensate</button><button className="danger-control" onClick={()=>onAction("archive",rule)}>Archive</button></div>}</section>}
function NotificationPanel({notifications,loading,onAction}:{notifications:Notification[];loading:string;onAction:(type:"create"|"resolve"|"send"|"retry"|"escalate"|"acknowledge"|"archive",item?:Notification)=>void}){const item=notifications.find(n=>n.status!=="ARCHIVED")??notifications[0],delivery=item&&Object.values(item.deliveries).at(-1);return <section className="panel"><div className="identity-heading"><div><p className="eyebrow">NOTIFICATION CENTER</p><h2>Delivery and acknowledgement</h2><p>Resolved recipients, multi-channel batches, retry failures, escalation, and acknowledgement.</p></div><button className="primary-button" disabled={Boolean(loading)} onClick={()=>onAction("create")}>＋ Create notice</button></div>{item?<div className="organization-overview"><article><small>Notice</small><strong>{item.title}</strong><span>{item.severity} · {item.status}</span></article><article><small>Recipients</small><strong>{Object.keys(item.recipients).length}</strong><span>{Object.values(item.recipients).filter(r=>r.acknowledged_at).length} acknowledged</span></article><article><small>Delivery</small><strong>{delivery?.status??"NONE"}</strong><span>{delivery?`retry ${delivery.retry_attempt}`:"not sent"}</span></article><article><small>Escalations</small><strong>{item.escalations.length}</strong><span>epoch {item.lifecycle_epoch}</span></article></div>:<p className="organization-placeholder">No notifications exist yet.</p>}{item&&item.status!=="ARCHIVED"&&<div className="organization-actions"><button onClick={()=>onAction("resolve",item)}>Resolve recipients</button><button onClick={()=>onAction("send",item)}>Send</button><button onClick={()=>onAction("retry",item)}>Retry delivery</button><button onClick={()=>onAction("escalate",item)}>Escalate</button><button onClick={()=>onAction("acknowledge",item)}>Acknowledge</button><button className="danger-control" onClick={()=>onAction("archive",item)}>Archive</button></div>}</section>}
function SynchronizationPanel({synchronizations,loading,onAction}:{synchronizations:Synchronization[];loading:string;onAction:(type:"start"|"offer"|"accept"|"merge"|"resolve"|"escalate"|"acknowledge"|"close",item?:Synchronization)=>void}){const item=synchronizations.find(s=>s.status!=="CLOSED")??synchronizations[0],open=item&&Object.values(item.conflicts).filter(c=>c.status!=="RESOLVED").length;return <section className="panel"><div className="identity-heading"><div><p className="eyebrow">SYNCHRONIZATION</p><h2>Replica convergence</h2><p>Operation batches, vector clocks, conflict resolution, escalation, and acknowledgement.</p></div><button className="primary-button" disabled={Boolean(loading)} onClick={()=>onAction("start")}>＋ Start sync</button></div>{item?<div className="organization-overview"><article><small>Status</small><strong>{item.status}</strong><span>version {item.version}</span></article><article><small>Batches</small><strong>{Object.keys(item.batches).length}</strong><span>operation exchange</span></article><article><small>Conflicts</small><strong>{open}</strong><span>open or escalated</span></article><article><small>Acks</small><strong>{Object.keys(item.acknowledgements).length}/2</strong><span>replicas</span></article></div>:<p className="organization-placeholder">No synchronizations exist yet.</p>}{item&&item.status!=="CLOSED"&&<div className="organization-actions"><button onClick={()=>onAction("offer",item)}>Offer batch</button><button onClick={()=>onAction("accept",item)}>Accept batch</button><button onClick={()=>onAction("merge",item)}>Merge batch</button><button onClick={()=>onAction("escalate",item)}>Escalate conflict</button><button onClick={()=>onAction("resolve",item)}>Resolve conflict</button><button onClick={()=>onAction("acknowledge",item)}>Acknowledge</button><button className="danger-control" onClick={()=>onAction("close",item)}>Close</button></div>}</section>}
function AuditPanel({partitions,loading,onAction}:{partitions:AuditPartition[];loading:string;onAction:(type:"append"|"seal"|"export"|"verify"|"archive",item?:AuditPartition)=>void}){const item=partitions.find(p=>p.status!=="ARCHIVED")??partitions[0];return <section className="panel"><div className="identity-heading"><div><p className="eyebrow">AUDIT LEDGER</p><h2>Integrity and retention</h2><p>Hash-chained entries, sealed partitions, bounded exports, verification, and archival.</p></div><button className="primary-button" disabled={Boolean(loading)} onClick={()=>onAction("append",item)}>＋ Append entry</button></div>{item?<div className="organization-overview"><article><small>Status</small><strong>{item.status}</strong><span>version {item.version}</span></article><article><small>Entries</small><strong>{item.entries.length}</strong><span>hash chained</span></article><article><small>Exports</small><strong>{Object.keys(item.exports).length}</strong><span>bounded ranges</span></article><article><small>Verifications</small><strong>{Object.keys(item.verifications).length}</strong><span>{item.root_digest.slice(0,8)}…</span></article></div>:<p className="organization-placeholder">No audit partition exists yet.</p>}{item&&item.status!=="ARCHIVED"&&<div className="organization-actions"><button onClick={()=>onAction("seal",item)}>Seal partition</button><button onClick={()=>onAction("export",item)}>Create export</button><button onClick={()=>onAction("verify",item)}>Verify integrity</button><button className="danger-control" onClick={()=>onAction("archive",item)}>Archive</button></div>}</section>}
function PolicyPanel({policies,loading,onAction}:{policies:Policy[];loading:string;onAction:(type:"create"|"version"|"publish"|"rate"|"evaluate"|"violation"|"hold"|"release"|"retire",item?:Policy)=>void}){const item=policies.find(p=>p.status!=="RETIRED")??policies[0];return <section className="panel"><div className="identity-heading"><div><p className="eyebrow">POLICY ENGINE</p><h2>Decisions, quota, and legal hold</h2><p title={POLICY_EVENT_LABELS.join(", ")}>Versioned rules, deterministic decisions, violations, holds, quota thresholds, and rate limiting.</p></div><button className="primary-button" disabled={Boolean(loading)} onClick={()=>onAction("create")}>＋ Create policy</button></div>{item?<div className="organization-overview"><article><small>Policy</small><strong>{item.name}</strong><span>{item.status} · {item.policy_type}</span></article><article><small>Versions</small><strong>{Object.keys(item.versions).length}</strong><span>{item.active_version_id?"published":"draft"}</span></article><article><small>Evaluations</small><strong>{Object.keys(item.evaluations).length}</strong><span>{Object.values(item.evaluations).filter(e=>e.decision==="DENY").length} denied</span></article><article><small>Controls</small><strong>{Object.keys(item.holds).length}</strong><span>{Object.keys(item.rate_limits).length} rate limits</span></article></div>:<p className="organization-placeholder">No policies exist yet.</p>}{item&&item.status!=="RETIRED"&&<div className="organization-actions"><button onClick={()=>onAction("version",item)}>Create version</button><button onClick={()=>onAction("publish",item)}>Publish</button><button onClick={()=>onAction("rate",item)}>Define rate limit</button><button onClick={()=>onAction("evaluate",item)}>Evaluate</button><button onClick={()=>onAction("violation",item)}>Register violation</button><button onClick={()=>onAction("hold",item)}>Apply legal hold</button><button onClick={()=>onAction("release",item)}>Release hold</button><button className="danger-control" onClick={()=>onAction("retire",item)}>Retire</button></div>}</section>}
...
function RecordDetail({ selection, history, missions, tasks, timelines, reports, loading, actionLoading, onTaskAction, onOpenMission, onOpenRecord, onClose }: { selection: RecordSelection; history: DomainEvent[]; missions: Mission[]; tasks: Task[]; timelines: Timeline[]; reports: Report[]; loading: boolean; actionLoading: string; onTaskAction: (type:"owner"|"priority"|"dependency"|"start" | "pause" | "block" | "submit" | "approve" | "close" | "reopen" | "cancel") => void; onOpenMission: (mission: Mission) => void; onOpenRecord: (selection: RecordSelection) => void; onClose: () => void }) {
  let id = "";
  let title = "";
  let description = "";
  let badge = "";
