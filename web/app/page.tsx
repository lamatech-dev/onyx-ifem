    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        <div className="content">
              ...
              <section className="dashboard-grid">
                <article className="panel mission-panel">
                  ...
                  <div className="mission-list">
                    {loading ? <LoadingRows /> : filteredMissions.length ? filteredMissions.slice(0, 5).map((mission, index) => <MissionRow key={mission.mission_id} mission={mission} index={index} taskCount={tasks.filter((task) => task.mission_id === mission.mission_id).length} />) : <EmptyState icon="◇" title="No missions yet" text="Create the first mission to activate your command center." action={() => setCreateKind("mission")} />}
                  </div>
                </article>
          ...
          {view !== "overview" && (
            <CollectionView view={view} missions={filteredMissions} tasks={tasks} timelines={timelines} reports={reports} loading={loading} onCreate={(kind) => setCreateKind(kind)} />
          )}
...
function MissionRow({ mission, index, taskCount }: { mission: Mission; index: number; taskCount: number }) {
  const colors = ["#7c6cf2", "#31b7c2", "#e19a43", "#59ad78", "#d76f91"];
  return <article className="mission-row"><div className="mission-code" style={{ background: `${colors[index % colors.length]}18`, color: colors[index % colors.length] }}>{String(index + 1).padStart(2, "0")}</div><div className="mission-main"><div><strong>{mission.title || "Untitled mission"}</strong><span className={statusClass(mission.status)}>{mission.status.replaceAll("_", " ")}</span></div><p>{mission.objective}</p><small>ID {shortId(mission.mission_id)} · version {mission.version}</small></div><div className="mission-stat"><strong>{taskCount}</strong><small>work items</small></div><button className="row-action" aria-label={`Open ${mission.title || "mission"}`}>›</button></article>;
}
...
function CollectionView({ view, missions, tasks, timelines, reports, loading, onCreate }: { view: Exclude<View, "overview">; missions: Mission[]; tasks: Task[]; timelines: Timeline[]; reports: Report[]; loading: boolean; onCreate: (kind: CreateKind) => void }) {
